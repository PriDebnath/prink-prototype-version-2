...
const drawPaths2 = (ctx: CanvasRenderingContext2D, state: CanvasState, appState: AppState) => {
  ctx.lineCap = "round";
  ctx.lineJoin = "round";
  ...
  for (const path of state.paths) {
    ...
    if(state?.selectedIds?.length){
      ...
      if (isSelected) {
        ...
        ctx.lineWidth = (path.pen.size / state.scale) + 4;
        ctx.strokeStyle =  "#2563EB";
        ctx.stroke();
        ctx.restore();
      }
...
const drawPaths3 = (ctx: CanvasRenderingContext2D, state: CanvasState, appState: AppState) => {
  ctx.lineCap = "round";
  ctx.lineJoin = "round";
  ...
  for (const path of state.paths) {
    ...
    if (isSelected) {
      ...
      ctx.globalAlpha = 1;                      // slightly transparent
      ctx.stroke();
      ctx.restore();
    }
